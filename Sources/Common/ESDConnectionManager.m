...
//         if(error == nil)
//        {
//            NSLog(@"Saved settings: %@", [NSString stringWithFormat:@"%@", jsonData]);
//        }
	}
}
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message
{
	@try
	{
        ...
        {
            //NSLog(@"received a sendToPlugin: %@", message);
            if([self.delegate respondsToSelector:@selector(sendToPlugin:withContext:withPayload:)])
            {
                [self.delegate sendToPlugin:action withContext:context withPayload:payload];
            }
        }
	}
	@catch(...)
