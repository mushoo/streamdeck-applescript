...
//         }
        if (jsonObj.event === 'sendToPropertyInspector') {
        	sendValueToPlugin('PI_Initialized', 'PIreceived');
            var payload = jsonObj.payload;
            if (payload.setInlineScript) 
            {
            	var sdpiScriptInline = document.getElementById('appleScriptInline');
            	sdpiScriptInline.value = payload.setInlineScript;
				sdpiScriptInline.disabled = false;
				sendValueToPlugin('PI_Initialized', 'set_up_inline');
                // Show Error
                // You can use this to show any errors and short circuit the rest of the refresh code
                //return;
            }
            if (payload.setScriptFile)
            {
            var elgScriptFile = document.getElementById('filepickergroup');
            elgScriptFile.value = payload.setScriptFile;
            elgScriptFile.label = payload.setScriptFile;
            elgScriptFile.disabled = false;
            
            }

        }
        //sendValueToPlugin('PI_Initialized', 'property_inspector');
...
function initPropertyInspector(initDelay) {
    ...
    setTimeout(function () {    }, initDelay || 100);
}
